...
public class CountryCollabController extends Table3Controller {
	...
	@FXML
	private TableColumn<CountryCollab, String> name;

	@FXML
	private TableColumn<CountryCollab, ScopusCountry> country1;

	@FXML
	private TableColumn<CountryCollab, ScopusCountry> country2;

	@FXML
	private TableColumn<CountryCollab, Integer> count;
	...
	public void setMainApp(AbstractJfxMainWindow app) {
		...
		country2.setCellFactory(ComboBoxTableCell.forTableColumn(mainApp.getScopusCountryData()));
		country2.setOnEditCommit(event -> {table.getSelectionModel().getSelectedItem().setCountry2(event.getNewValue()); mainApp.reset();});
	}

	public TableView<CountryCollab> getTable() {
		return table;
	}
	...
	private void initialize() {
		...
		choices.add("country1");
		country1.setCellValueFactory(new PropertyValueFactory<>("country1"));
		choices.add("country2");
	...
	public void filter(String attribute, String comparison, String text) {
		table.setItems(mainApp.getCountryCollabData());
		try {
			ObservableList<CountryCollab> filteredItems = FXCollections.observableArrayList();
			for (CountryCollab item : table.getItems()) {
