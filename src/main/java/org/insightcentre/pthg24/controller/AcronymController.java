...
public class AcronymController extends Table3Controller {
	...
	@FXML
	private TableColumn<Acronym, String> label;

	@FXML
	private TableColumn<Acronym, String> regExpr;

	@FXML
	private TableColumn<Acronym, Boolean> caseSensitive;
	...
	@FXML
	private TableColumn<Acronym, String> description;
	...
	public void setMainApp(AbstractJfxMainWindow app) {
		...
		ObservableList<ConceptType> conceptTypeValues = FXCollections.observableArrayList(ConceptType.values());
		conceptType.setCellFactory(ComboBoxTableCell.forTableColumn(conceptTypeValues));
		conceptType.setOnEditCommit(event -> {table.getSelectionModel().getSelectedItem().setConceptType(event.getNewValue()); mainApp.reset();});
	}

	public TableView<Acronym> getTable() {
		return table;
	}
	...
	public void filter(String attribute, String comparison, String text) {
		table.setItems(mainApp.getAcronymData());
		try {
			ObservableList<Acronym> filteredItems = FXCollections.observableArrayList();
			for (Acronym item : table.getItems()) {
	...
	class CaseSensitiveCallback implements Callback<TableColumn.CellDataFeatures<Acronym, Boolean>, ObservableValue<Boolean>> {
		@Override
		public ObservableValue<Boolean> call(TableColumn.CellDataFeatures<Acronym, Boolean> cellData) {
