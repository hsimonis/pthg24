 ...
 * Generated code
 */
public class NodeController extends Table3Controller {
	@FXML
	...
	@FXML
	private TableColumn<Node, Work> work;

	@FXML
	private TableColumn<Node, Integer> nr;

	@FXML
	private TableColumn<Node, Integer> nrEdges;

	@FXML
	private TableColumn<Node, Integer> connectedComponentNr;
	...
	public void setMainApp(AbstractJfxMainWindow app) {
		...
		table.setItems(mainApp.getNodeData());
	...
	public TableView<Node> getTable() {
		return table;
	}
	...
	private void initialize() {
		...
		choices.add("connectedComponentNr");
		connectedComponentNr.setCellValueFactory(new PropertyValueFactory<>("connectedComponentNr"));
		connectedComponentNr.setCellFactory(TextFieldTableCell.forTableColumn(INTEGER_CONVERTER));
		connectedComponentNr.setOnEditCommit(event -> {table.getSelectionModel().getSelectedItem().setConnectedComponentNr(event.getNewValue()); mainApp.reset();});
		initialize(choices);
	}
	...
	public void filter(String attribute, String comparison, String text) {
		table.setItems(mainApp.getNodeData());
		try {
			ObservableList<Node> filteredItems = FXCollections.observableArrayList();
