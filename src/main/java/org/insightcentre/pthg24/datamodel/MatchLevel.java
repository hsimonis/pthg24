...
Strong;
private static MatchLevel[] cache = null;

public static MatchLevel[] cachedValues(){
    if (cache== null){
        cache = MatchLevel.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[MatchLevel.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(MatchLevel p:MatchLevel.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[MatchLevel.cachedValues().length];
    int i=0;
    for(MatchLevel p:MatchLevel.cachedValues()){
        res[i++] = p.name();
    }
...
public static MatchLevel entry(int i){
...
public static int indexOf(MatchLevel p){
    return p.ordinal();
}
