...
public  class PhDThesis extends Work{
/**
 *  
 *
*/
...
*/
    ...
    public PhDThesis(ApplicationDataset applicationDataset){
        super(applicationDataset);
        setSchool(null);
        applicationDataset.addPhDThesis(this);
    }
...
*/
    ...
    public PhDThesis(PhDThesis other){
...
*/
    ...
    public Boolean remove(){
        ...
        return getApplicationDataset().removePhDThesis(this) && getApplicationDataset().removeWork(this) && getApplicationDataset().removeApplicationObject(this);
    }

/**
 *  get attribute school
 *
 * @return School
*/
...
/**
 *  set attribute school, mark dataset as dirty, mark dataset as not valid
@param school School
 *
*/
...
*/
     ...
     public void toXML(PrintWriter out){
         out.println("<phDThesis "+ " applicationDataset=\""+toXMLApplicationDataset()+"\""+
            " id=\""+toXMLId()+"\""+
            " name=\""+toXMLName()+"\""+
...
*/
    ...
    String toXMLSchool(){
        return "ID_"+this.getSchool().getId().toString();
    }
 ...
 * @param name PhDThesis name of the object we are looking for
 * @return PhDThesis entry of the dataset with the given name; otherwise null
*/

    public static PhDThesis findByName(ApplicationDataset base, String name){
        for(PhDThesis a:base.getListPhDThesis()) {
 ...
 * @param base  dataset in which we are searching
 * @param name PhDThesis name of the object we are looking for
 * @return PhDThesis entry of the dataset with the given name
*/

    public static PhDThesis findOrCreate(ApplicationDataset base, String name){
        ...
        PhDThesis res = new PhDThesis(base);
        res.setName(name);
        return res;
    }
 ...
 * @param base dataset in which we are searching
 * @return PhDThesis first entry in the dataset of this type; null if that does not exists
*/

    public static PhDThesis findFirst(ApplicationDataset base){
        if (base.getListPhDThesis().isEmpty()) {
            return null;
        }
        return base.getListPhDThesis().get(0);
    }

/**
 * find some entry entry in the dataset of that type
 * @param base dataset in which we are searching
 * @return PhDThesis some entry in the dataset of this type; null if that does not exists
*/

    public static PhDThesis findAny(ApplicationDataset base){
        int size=base.getListPhDThesis().size();
        if (size > 0) {
             return base.getListPhDThesis().get(new Random().nextInt(size));
        }
        return null;
    }
 ...
 * @param base dataset in which we are searching
 * @return PhDThesis last entry in the dataset of this type; null if that does not exists
*/

    public static PhDThesis findLast(ApplicationDataset base){
        int size=base.getListPhDThesis().size();
        if (size > 0) {
             return base.getListPhDThesis().get(size-1);
        }
        return null;
    }
 ...
 * @param b PhDThesis compare this to that object
 * @return Boolean true if the objects match the same criteria
*/

    public Boolean applicationSame(PhDThesis b){
...
*/
    ...
    public Boolean applicationEqual(PhDThesis b){
      ...
      if(!(getLink() == null ? b.getLink() == null:this.getLink().applicationSame(b.getLink()))){
      ...
      if(!this.getYear().equals(b.getYear())){
        ...
        return  this.getAbstractText().equals(b.getAbstractText()) &&
          ...
          (this.getLink() == null ? b.getLink() == null : this.getLink().applicationSame(b.getLink())) &&
...
*/
    ...
    public void check(){
        if (getApplicationDataset() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"applicationDataset","PhDThesis",(getApplicationDataset()==null?"null":getApplicationDataset().toString()),"",WarningType.NOTNULL);
        }
        if (getAuthors() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"authors","PhDThesis",(getAuthors()==null?"null":getAuthors().toString()),"",WarningType.NOTNULL);
        }
        if (getAuthors().size() == 0){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"authors","PhDThesis",(getAuthors()==null?"null":getAuthors().toString()),"",WarningType.NOTEMPTY);
        }
        if (getAwards() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"awards","PhDThesis",(getAwards()==null?"null":getAwards().toString()),"",WarningType.NOTNULL);
        }
        if (getConcept() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"concept","PhDThesis",(getConcept()==null?"null":getConcept().toString()),"",WarningType.NOTNULL);
        }
        if (getPublisher() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"publisher","PhDThesis",(getPublisher()==null?"null":getPublisher().toString()),"",WarningType.NOTNULL);
        }
        if (getSourceGroup() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"sourceGroup","PhDThesis",(getSourceGroup()==null?"null":getSourceGroup().toString()),"",WarningType.NOTNULL);
        }
        if (getSchool() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"school","PhDThesis",(getSchool()==null?"null":getSchool().toString()),"",WarningType.NOTNULL);
        }
    }

    static void dummy(ApplicationDataset base){
// no dummy information for class PhDThesis
    }
...
*/
   ...
   public List<ApplicationObjectInterface> getFeasibleValues(ApplicationDatasetInterface base,String attrName){
      ...
      if (attrName.equals("school")){
         return (List) ((Scenario)base).getListSchool();
      }
      return null;
   }
