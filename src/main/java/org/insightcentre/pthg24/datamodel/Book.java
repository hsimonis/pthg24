...
*/
    ...
    public Book(ApplicationDataset applicationDataset){
        super(applicationDataset);
        applicationDataset.addBook(this);
    }
...
*/
    ...
    public Boolean remove(){
        ...
        return getApplicationDataset().removeBook(this) && getApplicationDataset().removeWork(this) && getApplicationDataset().removeApplicationObject(this);
    }
...
*/
     ...
     public void toXML(PrintWriter out){
         out.println("<book "+ " applicationDataset=\""+toXMLApplicationDataset()+"\""+
            " id=\""+toXMLId()+"\""+
            " name=\""+toXMLName()+"\""+
 ...
 * @param name Book name of the object we are looking for
 * @return Book entry of the dataset with the given name; otherwise null
*/

    public static Book findByName(ApplicationDataset base, String name){
        for(Book a:base.getListBook()) {
 ...
 * @param base  dataset in which we are searching
 * @param name Book name of the object we are looking for
 * @return Book entry of the dataset with the given name
*/

    public static Book findOrCreate(ApplicationDataset base, String name){
 ...
 * @param base dataset in which we are searching
 * @return Book first entry in the dataset of this type; null if that does not exists
*/

    public static Book findFirst(ApplicationDataset base){
        if (base.getListBook().isEmpty()) {
            return null;
        }
        return base.getListBook().get(0);
    }

/**
 * find some entry entry in the dataset of that type
 * @param base dataset in which we are searching
 * @return Book some entry in the dataset of this type; null if that does not exists
*/

    public static Book findAny(ApplicationDataset base){
        int size=base.getListBook().size();
        if (size > 0) {
             return base.getListBook().get(new Random().nextInt(size));
        }
        return null;
    }
 ...
 * @param base dataset in which we are searching
 * @return Book last entry in the dataset of this type; null if that does not exists
*/

    public static Book findLast(ApplicationDataset base){
        int size=base.getListBook().size();
        if (size > 0) {
             return base.getListBook().get(size-1);
        }
        return null;
    }
 ...
 * @param b Book compare this to that object
 * @return Boolean true if the objects match the same criteria
*/

    public Boolean applicationSame(Book b){
...
*/
    ...
    public Boolean applicationEqual(Book b){
...
*/
    ...
    public void check(){
        if (getApplicationDataset() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"applicationDataset","Book",(getApplicationDataset()==null?"null":getApplicationDataset().toString()),"",WarningType.NOTNULL);
        }
        if (getAuthors() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"authors","Book",(getAuthors()==null?"null":getAuthors().toString()),"",WarningType.NOTNULL);
        }
        if (getAuthors().size() == 0){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"authors","Book",(getAuthors()==null?"null":getAuthors().toString()),"",WarningType.NOTEMPTY);
        }
        if (getAwards() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"awards","Book",(getAwards()==null?"null":getAwards().toString()),"",WarningType.NOTNULL);
        }
        if (getConcept() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"concept","Book",(getConcept()==null?"null":getConcept().toString()),"",WarningType.NOTNULL);
        }
        if (getPublisher() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"publisher","Book",(getPublisher()==null?"null":getPublisher().toString()),"",WarningType.NOTNULL);
        }
        if (getSourceGroup() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"sourceGroup","Book",(getSourceGroup()==null?"null":getSourceGroup().toString()),"",WarningType.NOTNULL);
        }
    }

    static void dummy(ApplicationDataset base){
// no dummy information for class Book
    }
