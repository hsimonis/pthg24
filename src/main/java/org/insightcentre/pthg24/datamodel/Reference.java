...
public  class Reference extends ApplicationObject{
/**
 *  
 *
*/
...
*/
    ...
    public Reference(ApplicationDataset applicationDataset){
        ...
        setOci("");
        setTimespan("");
        applicationDataset.addReference(this);
    }
...
*/
    ...
    public Boolean remove(){
        return getApplicationDataset().removeReference(this) && getApplicationDataset().removeApplicationObject(this);
    }
...
*/
     ...
     public void toXML(PrintWriter out){
         out.println("<reference "+ " applicationDataset=\""+toXMLApplicationDataset()+"\""+
            " id=\""+toXMLId()+"\""+
            " name=\""+toXMLName()+"\""+
...
*/
    ...
    public static String toHTMLLabels(){
        return "<tr><th>Reference</th>"+"<th>Name</th>"+"<th>Oci</th>"+"<th>CitedWork</th>"+"<th>CitingWork</th>"+"<th>Cited</th>"+"<th>Citing</th>"+"<th>Creation</th>"+"<th>Timespan</th>"+"<th>AuthorSC</th>"+"<th>JournalSC</th>"+"</tr>";
    }
 ...
 * @param a Reference item we are looking for
 * @param bList List<Reference> list of items in which we are searching
 * @return Reference entry of list b which is applicationSame() to a
*/

    public static Reference find(Reference a, List<Reference> bList){
 ...
 * @param name Reference name of the object we are looking for
 * @return Reference entry of the dataset with the given name; otherwise null
*/

    public static Reference findByName(ApplicationDataset base, String name){
        for(Reference a:base.getListReference()) {
 ...
 * @param base  dataset in which we are searching
 * @param name Reference name of the object we are looking for
 * @return Reference entry of the dataset with the given name
*/

    public static Reference findOrCreate(ApplicationDataset base, String name){
        ...
        Reference res = new Reference(base);
        res.setName(name);
        return res;
    }
 ...
 * @param base dataset in which we are searching
 * @return Reference first entry in the dataset of this type; null if that does not exists
*/

    public static Reference findFirst(ApplicationDataset base){
        if (base.getListReference().isEmpty()) {
            return null;
        }
        return base.getListReference().get(0);
    }

/**
 * find some entry entry in the dataset of that type
 * @param base dataset in which we are searching
 * @return Reference some entry in the dataset of this type; null if that does not exists
*/

    public static Reference findAny(ApplicationDataset base){
        int size=base.getListReference().size();
        if (size > 0) {
             return base.getListReference().get(new Random().nextInt(size));
        }
        return null;
    }
 ...
 * @param base dataset in which we are searching
 * @return Reference last entry in the dataset of this type; null if that does not exists
*/

    public static Reference findLast(ApplicationDataset base){
        int size=base.getListReference().size();
        if (size > 0) {
             return base.getListReference().get(size-1);
        }
        return null;
    }
 ...
 * @param b Reference compare this to that object
 * @return Boolean true if the objects match the same criteria
*/

    public Boolean applicationSame(Reference b){
...
*/
    ...
    public Boolean applicationEqual(Reference b){
...
*/
    ...
    public void check(){
        if (getApplicationDataset() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"applicationDataset","Reference",(getApplicationDataset()==null?"null":getApplicationDataset().toString()),"",WarningType.NOTNULL);
        }
        if (getCitedWork() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"citedWork","Reference",(getCitedWork()==null?"null":getCitedWork().toString()),"",WarningType.NOTNULL);
        }
        if (getCitingWork() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"citingWork","Reference",(getCitingWork()==null?"null":getCitingWork().toString()),"",WarningType.NOTNULL);
        }
    }

    static void dummy(ApplicationDataset base){
// no dummy information for class Reference
    }
