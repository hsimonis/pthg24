...
public  class ScopusCountry extends ApplicationObject{
/**
 *  
 *
*/
...
*/
    ...
    public ScopusCountry(ScopusCountry other){
 ...
 * @param a ScopusCountry item we are looking for
 * @param bList List<ScopusCountry> list of items in which we are searching
 * @return ScopusCountry entry of list b which is applicationSame() to a
*/

    public static ScopusCountry find(ScopusCountry a, List<ScopusCountry> bList){
 ...
 * @param name ScopusCountry name of the object we are looking for
 * @return ScopusCountry entry of the dataset with the given name; otherwise null
*/

    public static ScopusCountry findByName(ApplicationDataset base, String name){
        for(ScopusCountry a:base.getListScopusCountry()) {
 ...
 * @param base  dataset in which we are searching
 * @param name ScopusCountry name of the object we are looking for
 * @return ScopusCountry entry of the dataset with the given name
*/

    public static ScopusCountry findOrCreate(ApplicationDataset base, String name){
        ...
        ScopusCountry res = new ScopusCountry(base);
        res.setName(name);
        return res;
    }
 ...
 * @param base dataset in which we are searching
 * @return ScopusCountry first entry in the dataset of this type; null if that does not exists
*/

    public static ScopusCountry findFirst(ApplicationDataset base){
        if (base.getListScopusCountry().isEmpty()) {
            return null;
        }
        return base.getListScopusCountry().get(0);
    }

/**
 * find some entry entry in the dataset of that type
 * @param base dataset in which we are searching
 * @return ScopusCountry some entry in the dataset of this type; null if that does not exists
*/

    public static ScopusCountry findAny(ApplicationDataset base){
        int size=base.getListScopusCountry().size();
        if (size > 0) {
             return base.getListScopusCountry().get(new Random().nextInt(size));
        }
        return null;
    }
 ...
 * @param base dataset in which we are searching
 * @return ScopusCountry last entry in the dataset of this type; null if that does not exists
*/

    public static ScopusCountry findLast(ApplicationDataset base){
        int size=base.getListScopusCountry().size();
        if (size > 0) {
             return base.getListScopusCountry().get(size-1);
        }
        return null;
    }
 ...
 * @param b ScopusCountry compare this to that object
 * @return Boolean true if the objects match the same criteria
*/

    public Boolean applicationSame(ScopusCountry b){
...
*/
    ...
    public Boolean applicationEqual(ScopusCountry b){
...
*/
    ...
    public void check(){
        if (getApplicationDataset() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"applicationDataset","ScopusCountry",(getApplicationDataset()==null?"null":getApplicationDataset().toString()),"",WarningType.NOTNULL);
        }
    }

    static void dummy(ApplicationDataset base){
// no dummy information for class ScopusCountry
    }
