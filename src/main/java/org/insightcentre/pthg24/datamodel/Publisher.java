 ...
 * @param a Publisher item we are looking for
 * @param bList List<Publisher> list of items in which we are searching
 * @return Publisher entry of list b which is applicationSame() to a
*/

    public static Publisher find(Publisher a, List<Publisher> bList){
 ...
 * @param name Publisher name of the object we are looking for
 * @return Publisher entry of the dataset with the given name; otherwise null
*/

    public static Publisher findByName(ApplicationDataset base, String name){
        for(Publisher a:base.getListPublisher()) {
 ...
 * @param base  dataset in which we are searching
 * @param name Publisher name of the object we are looking for
 * @return Publisher entry of the dataset with the given name
*/

    public static Publisher findOrCreate(ApplicationDataset base, String name){
        ...
        Publisher res = new Publisher(base);
        res.setName(name);
        return res;
    }
 ...
 * @param base dataset in which we are searching
 * @return Publisher first entry in the dataset of this type; null if that does not exists
*/

    public static Publisher findFirst(ApplicationDataset base){
        if (base.getListPublisher().isEmpty()) {
            return null;
        }
        return base.getListPublisher().get(0);
    }

/**
 * find some entry entry in the dataset of that type
 * @param base dataset in which we are searching
 * @return Publisher some entry in the dataset of this type; null if that does not exists
*/

    public static Publisher findAny(ApplicationDataset base){
        int size=base.getListPublisher().size();
        if (size > 0) {
             return base.getListPublisher().get(new Random().nextInt(size));
        }
        return null;
    }
 ...
 * @param base dataset in which we are searching
 * @return Publisher last entry in the dataset of this type; null if that does not exists
*/

    public static Publisher findLast(ApplicationDataset base){
        int size=base.getListPublisher().size();
        if (size > 0) {
             return base.getListPublisher().get(size-1);
        }
        return null;
    }
 ...
 * @param b Publisher compare this to that object
 * @return Boolean true if the objects match the same criteria
*/

    public Boolean applicationSame(Publisher b){
...
*/
    ...
    public Boolean applicationEqual(Publisher b){
...
*/
    ...
    public void check(){
        if (getApplicationDataset() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"applicationDataset","Publisher",(getApplicationDataset()==null?"null":getApplicationDataset().toString()),"",WarningType.NOTNULL);
        }
    }

    static void dummy(ApplicationDataset base){
// no dummy information for class Publisher
    }
