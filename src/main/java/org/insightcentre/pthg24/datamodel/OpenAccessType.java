...
Gold;
private static OpenAccessType[] cache = null;

public static OpenAccessType[] cachedValues(){
    if (cache== null){
        cache = OpenAccessType.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[OpenAccessType.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(OpenAccessType p:OpenAccessType.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[OpenAccessType.cachedValues().length];
    int i=0;
    for(OpenAccessType p:OpenAccessType.cachedValues()){
        res[i++] = p.name();
    }
...
public static OpenAccessType entry(int i){
...
public static int indexOf(OpenAccessType p){
    return p.ordinal();
}
