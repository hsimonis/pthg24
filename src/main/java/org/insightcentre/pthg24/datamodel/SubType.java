...
public static SubType[] cachedValues(){
    if (cache== null){
        cache = SubType.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[SubType.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(SubType p:SubType.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[SubType.cachedValues().length];
    int i=0;
    for(SubType p:SubType.cachedValues()){
        res[i++] = p.name();
    }
...
public static SubType entry(int i){
...
public static int indexOf(SubType p){
    return p.ordinal();
}
