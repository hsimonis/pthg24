...
Other;
private static AwardLevel[] cache = null;

public static AwardLevel[] cachedValues(){
    if (cache== null){
        cache = AwardLevel.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[AwardLevel.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(AwardLevel p:AwardLevel.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[AwardLevel.cachedValues().length];
    int i=0;
    for(AwardLevel p:AwardLevel.cachedValues()){
        res[i++] = p.name();
    }
...
public static AwardLevel entry(int i){
...
public static int indexOf(AwardLevel p){
    return p.ordinal();
}
